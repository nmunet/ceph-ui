...
describe('CdValidators', () => {
  ...
  describe('ip', () => {
    it('should validate IPv4', () => {
      ...
      expect(validator(new FormControl('invalid'))).toEqual({ pattern: { requiredPattern: jasmine.any(String), actualValue: 'invalid' } });
    });
  ...
  describe('number', () => {
    it('should validate positive and negative numbers', () => {
      ...
      expect(validator(new FormControl('-123'))).toBeNull();
      expect(validator(new FormControl('abc'))).toEqual({ pattern: { requiredPattern: jasmine.any(String), actualValue: 'abc' } });
    });
    it('should validate only positive numbers', () => {
      const validator = CdValidators.number(false);
      expect(validator(new FormControl('123'))).toBeNull();
      expect(validator(new FormControl('-123'))).toEqual({ pattern: { requiredPattern: jasmine.any(String), actualValue: '-123' } });
    });
  });
  ...
  describe('decimalNumber', () => {
    ...
    it('should validate only positive decimals', () => {
      ...
      expect(validator(new FormControl('123.45'))).toBeNull();
      expect(validator(new FormControl('-123.45'))).toEqual({ pattern: { requiredPattern: jasmine.any(String), actualValue: '-123.45' } });
    });
  });
